...
| P2-003 / P2-018 / P2-004 (partial) | **Fixed / partial** | `refundPartialOrder` runs under the same order lock and rejects any refund exceeding the net paid sum (cumulative cap). True idempotency-key support for partial refunds (P2-004) still needs a `refunds` table / key column — see P4-028. |
...
| P2-004  | Critical | race-condition      | `apps/backend/src/orders/order-payment.service.ts:293-354` `refundPartialOrder` | No idempotency key + no row lock. Retry or double-click by the cashier creates a double-refund row. Needs idempotency-key support (idempotency-key header or `body.refundKey` lookup, unique index). |
| P2-005  | High     | wrong-calculation   | `apps/backend/src/orders/order-payment.service.ts:393-401` `adjustOrderItems` | Code comment in-place acknowledges the bug: *"In a full implementation we would recalculate tax, discount, tip perfectly."* When a manager adjusts items on a closed order, tax/discount/tip remain stale and only `subtotal` is updated. Customer-facing receipt and ledger diverge. **Block release.** |
...
| P3-001  | High     | leaked-secret       | `apps/backend/src/auth/auth.controller.ts:84-105` | `/auth/login` returns `refresh_token` in JSON body despite setting HttpOnly cookie — leak. |
| P3-002  | Critical | file-upload         | `apps/backend/src/menus/menus.controller.ts:223-246` | Unbounded, no MIME/fileFilter, preserves original extension → executable-as-PDF |
...
| P14-009 | Medium   | cache-invalidation| app.module.ts:46-58                                            | CacheManager wired but never read |
...
| ID      | Severity | Category                 | Location                                                       | Summary |
|---------|----------|--------------------------|----------------------------------------------------------------|---------|
| P3-001  | High     | leaked-secret            | `apps/backend/src/auth/auth.controller.ts:84-105` `login`      | `/auth/login` returns `{ access_token, refresh_token, user }` in the JSON body. The HttpOnly cookie is set, but ANY same-origin JS (or misconfigured browser-extensions, dev-tools logs to disk, proxies, error reporters) that can read fetch responses gets the refresh token, defeating the HttpOnly defense. Fix: return only `{ access_token, user }`; keep the cookie path as the only carrier for `refresh_token`. |
| P3-002  | Critical | file-upload              | `apps/backend/src/menus/menus.controller.ts:223-246` `uploadPdf` | Three compounding holes: (a) No `FileFilter` — `file.mimetype` is taken straight from the multipart header and is trivially spoofable. (b) No `multer` `limits.fileSize` configured → unbounded upload. (c) `const ext = file.originalname.split('.').pop()` is then **preserved** into the S3 object key extension (`.pdf.exe` keeps `.exe`). The server-side key prefix (time + random) prevents **path traversal** (good), but a malicious manager can store arbitrary `application/octet-stream` content as a `.exe` on the kitchen bucket. Fix: enforce `fileFilter` allowing only `application/pdf`, set `limits.fileSize: 20_000_000`, force `ext = 'pdf'` regardless of `originalname`. |
...
| P14-009 | Medium   | cache-invalidation | `@nestjs/cache-manager` registered globally in `apps/backend/src/app.module.ts:46-58` but **no decorator** (`@Cache`) is used. The Redis cache is wired but never read. Either remove from bootstrap debt or apply to read endpoints (`/health/version` is the only sensible target). |
