...
| P8-001  | High     | cross-tenant-read  | `apps/backend/src/calls/calls.service.ts:34-67` `listCalls`    | **RESOLVED 2026-07-13.** New private helper `resolveOrgScope(user)` returns `null` for platform-admins without `?orgId=`; `listCalls` short-circuits to `{ data: [], total: 0, hasMore: false }`. Platform-admins must explicitly pick a tenant via the `?orgId=` query string (validated by `JwtStrategy` UUID regex `?orgId=`). Other callers (`exportCallsCsv`, `exportCallsExcel`, `getCall*`) already use `user.organizationId` from the controller — not affected. |
...
| ID      | Severity | Category           | Location                                                       | Summary |
|---------|----------|--------------------|----------------------------------------------------------------|---------|
| P8-001  | High     | cross-tenant-read  | `apps/backend/src/calls/calls.service.ts:29-130` | Same as index row P8-001. **Fixed 2026-07-13.** |
